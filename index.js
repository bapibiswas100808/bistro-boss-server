...
app.use(
  cors({
    origin: [
      "http://localhost:5173",
      "http://localhost:5174",
      "https://bistro-boss100.web.app",
      "https://bistro-boss100.firebaseapp.com",
    ],
    credentials: true,
  })
);
...
const uri = `mongodb+srv://${process.env.DB_NAME}:${process.env.DB_PASS}@cluster0.5e8b5ac.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`;
...
async function run() {
  try {
    ...
    const menuCollections = client.db("BistroDb").collection("MenuCollections");
    ...
    // menu related api
    app.get("/MenuCollections", async (req, res) => {
      const result = await menuCollections.find().toArray();
      res.send(result);
    });
    app.get("/MenuCollections/:id", async (req, res) => {
      const id = req.params.id;
      console.log(id);
      const query = { _id: new ObjectId(id) };
    ...
    app.post("/MenuCollections", verifyToken, verifyAdmin, async (req, res) => {
      const menu = req.body;
      const result = await menuCollections.insertOne(menu);
      res.send(result);
    });
    app.patch("/MenuCollections/:id", async (req, res) => {
      const item = req.body;
      const id = req.params.id;
